...
> 状态：实现中 — axquery Phase 1-2 进行中（选择器 + Selection 核心 + 遍历 + 过滤 + 属性读取 + 迭代方法已完成）
> 日期：2026-03-26
> 最后更新：2026-03-26
...
当前覆盖率：**selector 97.1% / root 95.6% / 总计 ~96.1%**
...
| Level 1 | `axquery` | 🚧 实现中 | Phase 1 完成（选择器），Phase 2 进行中（Selection 核心 + 遍历 + 过滤 + 属性读取 + 迭代） |
