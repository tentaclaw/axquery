...
> 状态：实现中 — axquery Phase 1-2 完成（选择器 + Selection 全部），Phase 3 进行中（JS globals 注入完成，Selection 代理下一步）
> 日期：2026-03-26
> 最后更新：2026-03-26
...
- **JS 运行时层**：通过 `goja` 纯 Go JS 引擎实现，单元测试直接执行 JS 代码验证结果；Runtime 的 `Reset()` 支持测试隔离；`SystemBridge` 接口允许 fake bridge 替换 clipboard/keyboard 操作，实现纯行为测试
- **CGo 桥接层**（`appRootResolver`、`Query`）：薄封装，仅在集成测试中覆盖

当前覆盖率：**selector 97.1% / root 95.1% / js 95.5% / 总计 ~95.5%**
...
| Level 1 | `axquery` | 🚧 实现中 | Phase 1-2 完成（选择器 + Selection 全部），Phase 3 进行中（JS globals 注入完成） |
