...
> 状态：实现中 — axquery Phase 1-2 进行中（选择器 + Selection 核心 + 遍历 + 过滤 + 属性读取方法已完成）
> 日期：2026-03-26
> 最后更新：2026-03-26
...
当前覆盖率：**selector 97.1% / root 95.4% / 总计 ~96.0%**
...
| Level 1 | `axquery` | 🚧 实现中 | Phase 1 完成（选择器），Phase 2 进行中（Selection 核心 + 遍历 + 过滤 + 属性读取） |
