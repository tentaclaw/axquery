...
> 状态：实现中 — axquery Phase 1-2 完成（选择器 + Selection 全部），Phase 3 进行中（JS 运行时脚手架已完成）
> 日期：2026-03-26
> 最后更新：2026-03-26
...
- **JS 运行时层**：通过 `goja` 纯 Go JS 引擎实现，单元测试直接执行 JS 代码验证结果；Runtime 的 `Reset()` 支持测试隔离
- **CGo 桥接层**（`appRootResolver`、`Query`）：薄封装，仅在集成测试中覆盖

当前覆盖率：**selector 97.1% / root 95.1% / js 100% / 总计 ~95.8%**
...
| Level 1 | `axquery` | 🚧 实现中 | Phase 1-2 完成（选择器 + Selection 全部），Phase 3 进行中（JS 运行时脚手架） |
